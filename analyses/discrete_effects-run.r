...
out.dir <- project.path(paste("analyses/data/discrete_effects-simulations/", run, sep=""))
if (!file.exists(out.dir)) {
   dir.create(out.dir)
}

out <- t(sapply(run.res$fixations, unlist))
write.config.file(paste(out.dir, "/", run, "-before.rconf", sep=""), reg.mod.start)
write.config.file(paste(out.dir, "/", run, ".sconf", sep=""), sim.mod.start)
write.table(out, file=paste(out.dir, "/", run, ".out", sep=""), quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")
save(run.res, reg.mod.start, sim.mod.start, file=paste(out.dir, "/", run, ".rimage", sep=""))
write.config.file(paste(out.dir, "/", run, "-after.rconf", sep=""), run.res$mod.final)
